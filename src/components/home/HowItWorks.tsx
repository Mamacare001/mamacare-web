...
const meta = [
  { n: "01", icon: MessageSquareText, image: "/images/chat-on-phone.jpg", accent: "text-green" },
  { n: "02", icon: Brain, image: "/images/mother-home-phone.jpg", accent: "text-violet" },
  { n: "03", icon: Gauge, image: "/images/provider-tablet.jpg", accent: "text-violet" },
  { n: "04", icon: ClipboardCheck, image: "/images/telehealth-call.jpg", accent: "text-green" },
  { n: "05", icon: BellRing, image: "/images/chw-visit.jpg", accent: "text-coral" },
];
...
export function HowItWorks() {
  const { t } = useLang();
  const f = t.flow;
  const steps = f.steps.map((s, i) => ({ ...s, ...meta[i] }));
          ...
          <div className="relative lg:col-span-6">
            <div className="sticky top-28 aspect-[4/5] overflow-hidden rounded-xl shadow-float sm:aspect-[5/4] lg:aspect-[4/5]">
              ...
              <div className="absolute inset-x-5 bottom-5 flex items-center gap-3 rounded-lg bg-ivory/95 p-4 text-emerald shadow-soft backdrop-blur">
                ...
                <div>
                  <p className="text-eyebrow text-muted">{f.stepLabel} {current.n}</p>
